...
class Setup:
    ...
    def setup_simcam(self, login, password, directory):
        self.login = login
        ...
        try:
            ...
            filename = Path('../db/simdb.db')
            filename.touch(exist_ok=True)

            con = sqlite3.connect("../db/simdb.db")
            cur = con.cursor()
            ...
            if data is not None:
                ...
                con.commit()
            donnees = (login, password, directory)
            cur.execute("INSERT INTO setup (login, password, directory) VALUES (?, ?, ?)", donnees)
            con.commit()
            con.close()
        except sqlite3.Error as error:
            print("Failed to insert data into sqlite table", error)
            self.message = "Failed to insert data into sqlite table" + error
            self.status = False
